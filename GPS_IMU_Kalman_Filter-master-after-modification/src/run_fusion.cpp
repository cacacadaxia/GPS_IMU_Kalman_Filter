...
void GpsIns::loop()
{
	...
	FILE * fd3;
    fd3 = fopen("state6.txt","a+");
    Eigen::VectorXd zz = _sensor_data->get_state();
    fprintf(fd3,"%f\t%f\t%f\t%f\t%f\t%f\t", zz[0] , zz[1],zz[2],zz[3],zz[4],zz[5]);
	Eigen::VectorXd zz2 = _sensor_data->get_raw_data();
    fprintf(fd3,"%f\t%f\t%f\t%f\t%f\t%f\n", zz2[0] , zz2[1],zz2[2],zz2[3],zz2[4],zz[5]);
